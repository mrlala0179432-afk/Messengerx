...
module.exports.run = async function ({ api, event, args, Users }) {
  try {
    // 🔥 [অটোমেটিক ফরোয়ার্ড লজিক] ১ নম্বর গ্রুপে আসা সব মেসেজ সরাসরি ২ নম্বর গ্রুপে ফরোয়ার্ড হবে
    ...
    if (currentThreadID === targetGroup1 && String(event.senderID) !== botID) {
      const rawMsg = event.body ? event.body.trim() : "";
      let senderName = "User";
      try {
      ...
      console.log(`[FORWARD TRIGGER] Group 1 থেকে মেসেজ ধরা পড়েছে! Group 2-এ ফরওয়ার্ড করা হচ্ছে...`);

      let forwardContent = `📩 [Message from Group 1]\n👤 User: ${senderName} (${event.senderID})\n💬 Message: ${rawMsg || "Media/Attachment"}`;
        ...
        try {
          ...
          writer.on('finish', () => {
            api.sendMessage({ body: forwardContent, attachment: fs.createReadStream(tempPath) }, targetGroup2, (err, info) => {
              if (fs.existsSync(tempPath)) fs.unlinkSync(tempPath);
              if (err) {
                console.error("[FORWARD ERROR] মিডিয়া পাঠাতে সমস্যা:", err.message);
              } else {
          ...
          if (fs.existsSync(tempPath)) fs.unlinkSync(tempPath);
          console.error("[FORWARD CATCH ERROR]:", e.message);
        }
      } else {
        api.sendMessage(forwardContent, targetGroup2, (err, info) => {
          if (err) {
            console.error("[FORWARD ERROR] টেক্সট পাঠাতে সমস্যা:", err.message);
          } else {
      ...
      return; // গ্রুপ ১ থেকে মেসেজ সরাসরি ফরওয়ার্ড হয়ে যাওয়ায় বাকি চ্যাটবট রেসপন্স স্কিপ করা হলো
    }
...
module.exports.handleEvent = async function ({ api, event, Users }) {
  try {
    // ২ নম্বর গ্রুপ বা অন্য কোনো জায়গা থেকে ম্যানুয়াল কমান্ড চেক (#1, #2, #id)
    ...
    const greetings = [
      ...
      "எতো ডেকো না,প্রেম এ পরে যাবো তো🙈",
      ...
      "ভালোবাসার নামক আবলামি করতে চাইলে বস সাহুর ইনবক্সে গুতা দিন ~🙊😘🤣 𝐅𝐚𝐜𝐞𝐛𝐨𝐨𝐤 𝐋𝐈𝐍𝐊 : https://www.facebook.com/100044713412032",
      ...
      "-𝙂𝙖𝙮𝙚𝙨-🤗-যৌবনের কসম দিয়ে আমারে 𝐁𝐥𝐚𝐜𝐤𝗺𝗮𝗶𝐥 করা হচ্ছে-🥲🤦‍♂️🤧",
